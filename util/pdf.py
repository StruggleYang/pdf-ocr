...
company_keys = ["中国人民财产保险", "中国大地财产保险", "中华联合财产保险", "太平洋财产保险", "平安", "天安财产保险", "史带财产保险", "华安", "永安财产保险",
                "太平财产保险", "亚太", "美亚财产保险", "东京海上日动火灾保险", "瑞再企商保险", "安达保险", "三井住友海上火灾保险", "三星财产保险", "中银保险", "安联财产保险",
                "日本财产保险", "利宝保险", "安信农业保险", "中航安盟财产保险", "永诚财产保险", "安华农业保险", "安盛天平", "阳光财产保险", "阳光农业相互保险公司",
                "都邦", "渤海财产保险", "华农财产保险", "苏黎世财产保险", "中国人寿财产保险", "安诚", "现代财产保险", "长安", "劳合社保险", "中意财产保险",
                "爱和谊日生同和财产保险", "国元农业保险", "鼎和财产保险", "中煤财产保险", "国泰财产保险",
                "英大泰和财产保险", "紫金财产保险", "日本兴亚财产保险", "浙商财产保险", "国任财产保险", "乐爱金财产保险", "富邦财产保险", "信利保险", "泰山财产保险", "锦泰",
                "众诚汽车保险", "华泰", "长江", "诚泰", "安邦财产保险", "富德财产保险", "鑫安汽车保险", "北部湾财产保险", "众安",
...
match_keywords = {"客户": ["行驶证车主", "被保险人", "投保人姓名", "投保人名称", "投保人", "姓名"],
                  "身份证号": ["身份证号", "证件号码", "身份证", "营业执照"],
                  "签单日期": ["签单日期"],
                  "车牌号": ["车牌号", "号码号牌", "号牌号码", "车牌"],
                  "发动机号": ["发动机号码", "发动机号", "发动机"],
                  "车架号": ["车辆识别代码", "车架号", "识别代码"],
                  "车型": ["厂牌型号", "车型"],
                  "初登日期": ["初次", "首次", "登记日期"],
...
def get_insurant(keyword, keyword_full, rows, index):
    ...
    if '被保险' in keyword:
        if not '\n' in keyword_full:
            ...
            names = list(filter(
                lambda x: x != '被\n保\n险\n人' and x != '名 称' and x != '受益人', rows))
            if names:
                insurant = names[-1]
    # 被保险人既不是公司，名字长度却大于5或有换行，则认为无效
    if '公司' not in insurant and (len(insurant) > 5 or '\n' in insurant):
...
def get_plate_number(keyword):
    ...
    获取车牌号
    """
    keyword = remove_blank(keyword).replace("-", "")
    plate_number = ''
    # 单号的规则可能会影响到车牌识别
    if '单号' in keyword or '厂牌型号' in keyword or '车架号' in keyword:
        return plate_number
    # ===========非单元格内容匹配============
...
def read_pdf(file, all_customer):
        ...
        with pdfplumber.open(file) as pdf:
            ...
            for page in pdf.pages:
                ...
                if not temp_customer.insurance_categories:
                    ...
                    elif '人身意外伤害' in all_content or '意外伤害' in all_content or '意外险' in all_content:
            ...
            # 且信息没有获取到，需要从完整文本中获取
            if not temp_customer.id_number or (not temp_customer.insurant and not temp_customer.plate_number):
                for cache_content in page_content_cache:
                    # if '潘丽红云PTD032驾乘无忧' in file:
                    #     print(cache_content)
    ...
    if not match_customer and temp_customer.plate_number:
        match_customer = list(
